...
class Calculator(private val numberPrecision: Int) {

    // tinh giai thua

    fun factorial(number: BigDecimal): BigDecimal {
        if (number >= BigDecimal(3000)) {
            println("callallalalala2")
    ...
    private fun exponentiation(x: BigDecimal, parseFactor: BigDecimal): BigDecimal {
        ...
        if (value == BigDecimal.ZERO) {
            ...
            if (parseFactor > BigDecimal(999)) {
                println("callallalalala1")
                is_infinity = true
                value = BigDecimal.ZERO
            } else {
            ...
            fun parseFactor(): BigDecimal {
                ...
                if (eat('('.code)) { // parentheses
                    ...
                    when (func) {
                        ...
                        "sinh" -> {
                            Log.d("Calculation", "sinh")
                            x = sinh(x.toDouble()).toBigDecimal()
                            if (x > BigDecimal.ZERO && x < BigDecimal(1.0E-14)) {
                                x = round(x.toDouble()).toBigDecimal()
                            }
                        }
                        ...
                        "cosh" -> {
                            Log.d("Calculation", "cosh")
                            x = cosh(x.toDouble()).toBigDecimal()
                            if (x > BigDecimal.ZERO && x < BigDecimal(1.0E-14)) {
                                x = round(x.toDouble()).toBigDecimal()
                            }
                        }
    ...
    // Thêm vào lớp Calculator
    private val PHI = BigDecimal((1 + sqrt(5.0)) / 2)
}
